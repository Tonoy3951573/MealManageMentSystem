...
void register_student(){
    ...
    printf("Name : ");
    scanf(" %[^\n]",student.name);
    if(strcmp(student.name,":b") == 0) return;

    printf("Roll no : ");
    scanf("%s",student.roll_no);
    if(strcmp(student.roll_no,":b") == 0) return;

    printf("Room Number : ");
    char temp[MAX_LEN];
    scanf(" %[^\n]",temp);
    if(strcmp(temp,":b") == 0) return;
    student.room_number = atoi(temp);

    printf("Password : ");
    scanf("%s",passKey.password);
    if(strcmp(passKey.password,":b") == 0) return;
...
void printCalender(int arr[]){
    char stat[8][5] = {"OFF"," B "," L ","BL "," D ","BD ","LD ","BLD"};
    for(int i = 0; i < 4; i++){
        for(int j = 0; j < 7; j++){
            int day = i*7 + j ;
            if(day > getDay() - 1) printf("  .  ");
            else{
                printf(" %s ",stat[arr[day]]); 
            }
        }
        printf("\n");
    }
...
int update_room_number(Students Students[], int ind){
    ...
    Students[ind].room_number = atoi(temp);
    return 1;
}
...
void view_profile(char* roll_no){
    ...
    while (running) {
        ...
        if(choice == 1){
            ...
            printf("Please Select 1, 2, 4, 5, 6, 7, or 8.\n");
            sleep(1);
        }
    }
}
